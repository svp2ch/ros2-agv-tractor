...
def generate_launch_description():
    ...
    lidar_processor_node = Node(
        ...
        parameters=[{
            'use_sim_time': True,
            # min_range=1.0: отсекаем самозасветку корпусом. Лидар вынесен на
            # 0.55м вперёд при robot_radius=1.5м, поэтому реальное препятствие
            # физически не может быть ближе ~0.95м (оно было бы внутри трактора).
            # Всё ближе 1.0м — отражения от собственной кабины (особенно когда
            # трактор кренится на развороте и плоскость скана задевает крышу).
            'min_range':       1.0,
            'max_range':       19.0,   # диагональ поля ~21м, 19м хватает
            'grid_resolution': 0.1,
            'grid_cells':      200,
        }],
        remappings=[
